...
// 确保 PluginManager 实现了 plugin.PluginManager 接口
var _ plugin.PluginManager = (*PluginManager)(nil)

// PluginManager 插件管理器，负责插件的注册、加载和事件分发
type PluginManager struct {
	plugins map[string]plugin.Plugin // 插件映射表，key为插件名称
...
// 如果任何插件加载失败，返回第一个遇到的错误
func (pm *PluginManager) LoadPlugins() error {
	pm.mu.RLock()
	pluginList := make([]plugin.Plugin, 0, len(pm.plugins))
	for _, p := range pm.plugins {
		pluginList = append(pluginList, p)
	}
	pm.mu.RUnlock()

	pm.logger.Info().Int("插件数量", len(pluginList)).Msg("加载插件")
	...
	for _, p := range pluginList {
		...
		if err := p.Load(); err != nil {
			pm.logger.Error().
				...
				Str("插件名", metadata.Name).
				Msg("插件加载失败")
			return fmt.Errorf("插件 %s 加载失败: %w", metadata.Name, err)
		}
	...
	pm.logger.Info().Int("已加载", loadedCount).Msg("所有插件加载完成")
	return nil
}
